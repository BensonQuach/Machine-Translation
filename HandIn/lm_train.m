...
for iFile=1:length(DD)
  ...
  for l=1:length(lines)
    ...
    % lm_train('/u/cs401/A2_SMT/data/Hansard/Training/', 'e', '/h/u6/g0/00/g0quachb/Desktop/A2-401/en_LM')
    % lm_train('/u/cs401/A2_SMT/data/Hansard/Training/', 'f', '/h/u6/g0/00/g0quachb/Desktop/A2-401/fr_LM')
    ...
    for k=1:length(words)
        ...
        % Build the bigram
        if not(strcmp(currWord, 'SENTSTART'))
            prevWord = char(words(k - 1));
            if isfield(LM.bi, prevWord) && isfield(LM.bi.(prevWord), currWord)
                LM.bi.(prevWord).(currWord) = LM.bi.(prevWord).(currWord) + 1;
            else
                LM.bi.(prevWord).(currWord) = 1;
            end
        end
    end
